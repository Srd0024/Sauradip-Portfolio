      ...
      <div className="max-w-7xl mx-auto py-12 sm:py-16 px-4 sm:px-8">
        <div className="relative">
          <div 
            ...
            className="flex lg:grid lg:grid-cols-3 gap-6 sm:gap-8 overflow-x-auto lg:overflow-visible snap-x snap-mandatory [&::-webkit-scrollbar]:hidden"
            style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
          >
            {TESTIMONIALS.map((test) => (
              <div
                key={test.id}
                className="w-full flex-none lg:w-auto lg:flex-auto snap-center bg-white p-6 sm:p-8 rounded-3xl border border-neutral-200/90 shadow-editorial shadow-editorial-hover relative flex flex-col justify-between"
              >
                <div>
                ...
                {/* Author Info */}
                <div className="pt-5 border-t border-neutral-100 flex items-center gap-3 mt-auto">
            ...
            {TESTIMONIALS.map((_, index) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-colors ${
                  activeIndex === index ? 'bg-[#E8874A]' : 'bg-neutral-200'
                }`}
